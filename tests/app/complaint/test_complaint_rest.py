...
def test_get_all_complaints_returns_complaints_for_multiple_services(client, notify_db_session):
    ...
    response = client.get('/complaint', headers=[create_service_authorization_header()])
...
def test_get_all_complaints_returns_pagination_links(mocker, client, notify_db_session):
    ...
    response = client.get(url_for('complaint.get_all_complaints', page=2), headers=[create_service_authorization_header()])
...
def test_get_complaint_sets_start_and_end_date_to_today_if_not_specified(mocker, client):
    ...
    response = client.get(url_for('complaint.get_complaint_count'), headers=[create_service_authorization_header()])
