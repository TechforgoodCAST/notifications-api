...
def test_get_all_letter_brands(client, notify_db_session):
    ...
    response = client.get('/letter-branding', headers=[create_service_authorization_header()])
...
def test_get_letter_branding_by_id(client, notify_db_session):
    ...
    response = client.get('/letter-branding/{}'.format(hm_gov.id), headers=[create_service_authorization_header()])
...
def test_get_letter_branding_by_id_returns_404_if_does_not_exist(client, notify_db_session):
    response = client.get('/letter-branding/{}'.format(uuid.uuid4()), headers=[create_service_authorization_header()])
